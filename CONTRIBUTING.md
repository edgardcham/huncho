...
- Vendor dialects are specified by fixtures under `fixtures/wires/`. Policy semantics are specified by `fixtures/policy/*.json`; those files are the spec a port in another language must pass. Each file is `{ clauses, sequence }`. A clause is `numeric` (`select` key, `enter`, optional `exit`, `outcome`), `boolean` (`test` key, optional `exit` key, `outcome`), or `else` (`outcome`). `answers` is a flat object of numbers and booleans; a numeric `select` reads a number, a boolean `test` or `exit` reads a boolean. Each sequence step is `{ answers, previous?, expect, via? }`; a runner checks `via` (`enter`, `hold` or `else`) when present. Same clauses and answers, same outcome.
